...
namespace EncryptDecryptConfig
{
	...
	public class Crypto
	{
        ...
        #region Encrypt File
        public static bool EncryptFile(string Filename1, string Filename2, string EncryptionKey)
        {
            bool Ret = false;
            string Contents = "";
            ...
            try
            {
                ...
                oLocalFile.Close();
                byte[] encryptData = Crypto.Encrypt(EncryptionKey, encoding.GetBytes(Contents));
        ...
        #region Decrypt File
        public static bool DecryptFile(string Filename1, string Filename2, string EncryptionKey)
        {
            bool Ret = false;
            ASCIIEncoding encoding = new ASCIIEncoding();
            try
            {
                ...
                byte[] decryptedData = Crypto.Decrypt(EncryptionKey, r.ReadBytes((int)fr.Length));
                r.Close();
                fr.Close();
