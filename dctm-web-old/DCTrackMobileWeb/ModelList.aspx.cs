...
public partial class ModelList : System.Web.UI.Page
{
    ...
    private void LoadModels(string MfgName, string MfgID, string BUID)
    {
        ...
        //webdatatree1.getnode(0).get_childnode(0).remove
    ...
    protected void btnOK_Click(object sender, Infragistics.WebUI.WebDataInput.ButtonEventArgs e)
    {
        foreach (DataTreeNode node in TreeLocation.CheckedNodes)
        {
            if (!modelList.Contains(node.Text))
            {
                modelList.Add(node.Text);
            }
        }
        list = string.Join(",", (string[])modelList.ToArray(typeof(string)));
        //list = "DL380,DL380G4";
        
       Page.ClientScript.RegisterStartupScript(typeof(string),"myScript", "<script language=JavaScript>nodeClicked();</script>");
