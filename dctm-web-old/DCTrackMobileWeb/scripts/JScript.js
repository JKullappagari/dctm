...
function open_window(link, windowName, settings) {
    var win = window.open(link, windowName, settings);
    return win;
}
...
if (window.body) {
    window.body.onbeforeunload = confirmExit; // IE
}
else {
    window.onbeforeunload = confirmExit;   // FX
}
...
function confirmExit() {
    ...
    // interval is set for 120 seconds
    if (needToConfirm) {
        window.unloadTimer = setInterval(
    "window.parent.stayOnPage();", 120000);
...
function ValidateMapNew(oButton, oEvent, sMessage) {
    var theBox;
    var IsChecked = false;
    var elm;
    ...
    if (icheck == 0) {
        alert("Please select at least one item to Map");
        oEvent.cancel = true;
        //return false;
    }
    else {
...
//added by kjb on 13th Aug 2011
function ValidateDeletionApplicationMap(oButton, oEvent, sMessage) {
    var cnf = window.confirm(sMessage);
    if (cnf)
        oEvent.cancel = false;
    else
...
//Start - ValidateDeletionForRegDev
function ValidateDeletionForRegDev(oButton, oEvent, sMessage) {
    var theBox;
    var IsChecked = false;
    var elm;
    ...
    if (icheck == 0) {
        alert("Please select at least one device to disable");
        oEvent.cancel = true;
        //return false;
    }
    else {
...
//End - ValidateDeletionForRegDev

//start - validate print
function ValidatePrint(oButton, oEvent) {
    var theBox;
    var IsChecked = false;
    var elm;
    ...
    for (i = 0; i < elm.length; i++) {
        if (elm[i].type == "checkbox" && elm[i].id != theBox.id) {
            sItemName = elm[i].name;
            if (sItemName.indexOf("ChkSelect", 0) >= 0) {
                if (elm[i].checked == true) {
                    IsChecked = true;
                    icheck += 1;
                    icount += 1;
    ...
    if (icheck == 0) {
        ...
        var cnf = window.confirm("Are you sure you want to print?");
        if (cnf)
            oEvent.cancel = false;
        else
            oEvent.cancel = true;
...
function doKeypress(control, maxLength,event) {
    value = control.value;
...
function doBeforePaste(control, maxLength,event) {
    if (maxLength) {
        event.returnValue = false;
    }
}
// Cancel default behavior and create a new paste routine
function doPaste(control, maxLength,event) {
    value = control.value;
    if (maxLength) {
        event.returnValue = false;
        maxLength = parseInt(maxLength);
