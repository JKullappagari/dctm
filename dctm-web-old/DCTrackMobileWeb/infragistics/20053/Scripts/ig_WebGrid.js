...
var igtbl_sUnmaskedValue="unmaskedValue";
var igtbl_sigCellText="igCellText";
var igtbl_sigDataValue="igDataValue";
...
function igtbl_getBandById(tagId) 
{
	...
	else if(el && gridId.charAt(gridId.length-2)=="r" && gridId.charAt(gridId.length-1)=="c" && el.tagName=="TD")
		gridId=gridId.substr(0,gridId.length-2);
	else if(el && (gridId.charAt(gridId.length-2)=="g" && gridId.charAt(gridId.length-1)=="r" && el.getAttribute("groupRow") || gridId.charAt(gridId.length-2)=="r" && gridId.charAt(gridId.length-1)=="h" && el.getAttribute("hiddenRow")))
		gridId=gridId.substr(0,gridId.length-2);
	else if(el && gridId.charAt(gridId.length-3)=="a" && gridId.charAt(gridId.length-2)=="n" && gridId.charAt(gridId.length-1)=="c" && el.tagName=="TD")
		gridId=gridId.substr(0,gridId.length-3);
...
function igtbl_getColumnById(tagId) 
{
	...
	if(gridId.length>=3 && gridId.charAt(gridId.length-3)=="a" && gridId.charAt(gridId.length-2)=="n" && gridId.charAt(gridId.length-1)=="c" && el && el.tagName=="TD")
	{
		gridId=gridId.substr(0,gridId.length-3);
...
if(gridId.charAt(gridId.length-2)=="r" && gridId.charAt(gridId.length-1)=="c" && el && el.tagName=="TD")
	...
	else if(gridId.charAt(gridId.length-1)=="c" || gridId.charAt(gridId.length-1)=="f")
	{
		if(el && el.tagName!="TH")
			return null;
		gridId=gridId.substr(0,gridId.length-1);
...
function igtbl_getRowById(tagId) 
{
	...
	if(gridId.length>=4 && gridId.charAt(gridId.length-4)=="a" && gridId.charAt(gridId.length-3)=="n" && gridId.charAt(gridId.length-2)=="f" && gridId.charAt(gridId.length-1)=="r")
	{
		row=igtbl_getWorkRow(igtbl_getElementById(tagId).parentNode.parentNode.parentNode.parentNode.parentNode);
		if(!row || row.tagName!="TR")
			row=null;
		else
			gridId=gridId.substr(0,gridId.length-4);
	...
	if(row==null && gridId.charAt(gridId.length-3)=="g" && gridId.charAt(gridId.length-2)=="r" && gridId.charAt(gridId.length-1)=="c")
	{
	...
	if(row==null && gridId.charAt(gridId.length-3)=="n" && gridId.charAt(gridId.length-2)=="f" && gridId.charAt(gridId.length-1)=="r")
	{
		row=igtbl_getWorkRow(igtbl_getElementById(tagId).parentNode.parentNode.parentNode.parentNode.parentNode);
		if(!row || row.tagName!="TR")
			row=null;
	...
	if(row==null && gridId.charAt(gridId.length-3)=="a" && gridId.charAt(gridId.length-2)=="n" && gridId.charAt(gridId.length-1)=="r")
	{
		row=igtbl_getElementById(tagId);
		if(!row || row.tagName!="TR")
			row=null;
	...
	if(row==null && gridId.charAt(gridId.length-3)=="a" && gridId.charAt(gridId.length-2)=="n" && gridId.charAt(gridId.length-1)=="l")
	{
		row=igtbl_getElementById(tagId);
		if(typeof(row)!="undefined" && row)
			row=row.parentNode;
	...
	if(row==null && gridId.charAt(gridId.length-3)=="a" && gridId.charAt(gridId.length-2)=="n" && gridId.charAt(gridId.length-1)=="c")
	{
		row=igtbl_getElementById(tagId);
		if(typeof(row)!="undefined" && row)
			row=row.parentNode;
		if(row && row.id.substr(0,gridId.length+1)==gridId.substr(0,gridId.length-3)+"anfr")
			do{
				row=row.parentNode
	...
	if(row==null && gridId.charAt(gridId.length-2)=="r" && gridId.charAt(gridId.length-1)=="c")
	{
		...
		if(row && row.id.substr(0,gridId.length+1)==gridId.substr(0,gridId.length-2)+"nfr")
			do{
				row=row.parentNode
			}while(row && row.tagName!="TR");
		if(!row || row.tagName!="TR")
...
function igtbl_getCellById(tagId) 
{
	...
	{
	if(!(gridId.charAt(gridId.length-3)=="a" && gridId.charAt(gridId.length-2)=="n" && gridId.charAt(gridId.length-1)=="c"))	
			return null;
		gridId=gridId.substr(0,gridId.length-3);
	}
	else
...
function igtbl_headerClickDown(evnt,gn) 
{
	...
	if(!gn || !evnt)
		return false;
	var gs=igtbl_getGridById(gn);
	if(!gs || gs._thisForm && gs._thisForm.disabled)
		return;
	gs.event=evnt;
...
function igtbl_headerMouseOut(evnt,gn) 
{
	...
	var gs=igtbl_getGridById(gn);
	var se=igtbl_srcElement(evnt);
	if(!gs || !se || gs._thisForm && gs._thisForm.disabled)
		return;
	gs.event=evnt;
			...
			)
			{
				var visElem=null;
					for(var j=fac;j<row.cells.length && !visElem;j++)
					{
						...
						{
							cida=cida.split("_");
							if(cida[cida.length-1]==colIdA[colIdA.length-1])
								visElem=cell;
						}
						else 
						if(cell.firstChild && cell.firstChild.id==gn+"_drs")
						{
							...
							for(var k=0;k<row.cells.length && !visElem;k++)
							{
								cell=row.cells[k];
								cida=cell.id;
								if(cida)
...
function igtbl_colButtonClick(evnt,gn,b,se)
{
	...
	var gs=igtbl_getGridById(gn);
	if(gs==null||gs._exitEditCancel||se==null||se.id=="" || gs._thisForm && gs._thisForm.disabled)
		return;
	gs.event=evnt;
...
{
	var gs=igtbl_getGridById(gn);
	if(gs==null || gs._thisForm && gs._thisForm.disabled)return;
...
function igtbl_getWorkRow(row,gridId)
{
	...
	if(row.getAttribute("groupRow"))
	{
		...
		{
			var rowId = id[0].replace(gridId,"");
			if (rowId.length==3&&rowId=="sgr")
				return row.parentNode.parentNode.parentNode.parentNode;
			else
				return row;
		}
...
{
	var gs=igtbl_getGridById(gn);
	if(!gs || !gs._scrElem || gs._thisForm && gs._thisForm.disabled) return;
	if(evnt.wheelDelta)
	{
		igtbl_hideEdit(gn);
