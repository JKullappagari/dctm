...
function igtbl_editCell(evnt,gn,cell,key)
{
	...
	var elem=igtbl_editCust(g,(key!=null&&key>0)?"":v,r,col,cElem);
	if(!elem)
		elem=igtbl_editList(g,v=(cell.getAttribute(igtbl_sigDataValue)?cell.getAttribute(igtbl_sigDataValue):v.toString()),r,col,cElem);
	if(!elem)
		elem=igtbl_editDef(g,v=(cell.getAttribute(igtbl_sigDataValue)?cell.getAttribute(igtbl_sigDataValue):v.toString()),r,col,css,cell,cElem);
	if(!elem)
		return;
	cellObj._oldValue=v;
	if(href>=0)
