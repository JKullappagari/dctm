...
//** END OBSOLETE ***
	...
	{		
		...
		if(ig_csom.IsIE)
		{
			...
			this.Url=document.URLUnencoded;
			    ...
			    this.XmlHttp=ig_createActiveXFromProgIDs(["MSXML2.XMLHTTP","Microsoft.XMLHTTP"]);
	...
	if(thisForm)
	{
		...
		else
		{
			...
			if(window.__doPostBack!=igtbl_submit)
			{
...
{
	{
		...
		if(this.fireEvent(this.Events.BeforeXmlHttpRequest,[this.Id])==true)
		{
			this.ReqType=this.eReqType.None;
			this.ReadyState=this.eReadyState.Ready;
			return;
...
function(scrollLeft,force)
{
	...
	if(!divs.firstChild.style.width && this.Element.offsetWidth)
	{
		...
		divs.firstChild.style.width=(this.Element.offsetWidth + (this.GroupCount==1?this.Bands[0].getIndentation():0)+1+corr).toString()+"px";
	...
	else
	{
	    ...
	    var oldFixedHeadersValue=this.UseFixedHeaders;
		this.UseFixedHeaders=false;
		igtbl_scrollLeft(divs,scrollLeft);
		this.UseFixedHeaders=oldFixedHeadersValue;
	}
	var doHoriz=false;
...
function()
{
	...
	if(!this.Bands[0].IsGrouped && this.StatHeader && (this.StationaryMargins==1 || this.StationaryMargins==3))
	{
		...
		))
		{
			...
			if(this.MainGrid && this.MainGrid.style.height)
			{
...
function()
{
	var result=this.GridIsLoaded;
	if(this._thisForm && this._thisForm.disabled)
		result=false;
	return result;
},
...
function(element,node,band,parentRow)
{
	...
	{
		var anr=igtbl_getElementById(this.Grid.Id+"anr"+(this.ParentRow?"_"+this.ParentRow.getLevel(true):""));
		if(anr)
...
)
{
	...
	if(hasGrouped)
	{
		...
		if(!this.Band._wdth)
		{
			...
			for(var i=0;i<this.Band.Columns.length;i++)
				if(this.Band.Columns[i].getVisible())
				{
					...
					if(colWidth && typeof(colWidth)=="string" && (colWidth.length<=2 || colWidth.substr(colWidth.length-2)!="px"))
					{
						wdth=0;
						break;
					}
		...
		xslProc.addParameter("grpWidth",this.Band._wdth);
	}
...
//***********
	...
	{
		if(!this.Expandable)
		this.Expandable=this.Node.selectSingleNode("Rows")!=null || this.Node.getAttribute("showExpand")=="true";
			
	}
	if(this.Node)
	{
		
		var dataKey=unescape(this.get(igtbl_litPrefix+"DataKey"));
...
function(expand)
{
	...
	if(gs._thisForm && gs._thisForm.disabled) return;
		...
		else
		{
			if(!band.HeaderHTML)
			{
				...
				for(var i=0;i<band.Columns.length;i++)
				{
					...
					{
						th=document.createElement("th");
						th.id=this.gridId+"c"+"_"+band.Index+"_"+i.toString();
				...
				for(var i=0;i<band.Columns.length;i++)
				{
					...
					if(column.hasCells())
					{
						...
						if(g.UseFixedHeaders && !column.Fixed && !nfrow)
						{
							...
							nftable.appendChild(nftb);
							nfrow=document.createElement("tr");
							nfrow.id=this.gridId+"anfr_"+this.getLevel(true);
							nftb.appendChild(nfrow);
						}
		...
		if(this.Node)
		{
			...
			while(colNo<col.Node.parentNode.childNodes.length)
			{
				if(!col.Node.parentNode.childNodes[colNo].getAttribute("serverOnly"))
					cni++;
				if(colNo==col.Node.firstChild.getAttribute(igtbl_litPrefix+"columnNo"))
					break;
				colNo++;
			}
			if(cni>=0 && cni<col.Node.parentNode.childNodes.length)
				node=this.Node.selectSingleNode("Cells").childNodes[cni];
		}
	...
	if(this.Element)
	{
		...
		else 
		{
			...
			{				
				...
				if(this.Node)
				{
					this.Node.selectSingleNode(
						"Content"
						).firstChild.text=(e.getAttribute(igtbl_sUnmaskedValue,"")=="")?"&nbsp;":vs;
				}		
			}
		}
	}
...
{
	return unescape(node.selectSingleNode("Value").text);
}
function igtbl_setNodeValue(node,value)
{
	return node.selectSingleNode("Value").text=igtbl_escape(value);
}
...
function igtbl_changeBorder(g,elemFrom,elemTo,obj,attr,render)
{
	...
	if(render==false)
	{
		...
		for(var i=0;i<attrs.length;i++)
			try{delete obj[attrs[i]];}catch(e){;}
	}
	else
	{
		if(typeof(obj["old"+attrStyleSel])=="undefined"
