...
<xsl:template match="wbg:Row">
	...
	<tr id="{$gridName}r_{$parentRowLevel}{$rowIndex}" level="{$parentRowLevel}{$rowIndex}">
      ...
      <xsl:if test="@lit:rowStyle">
        <xsl:value-of select="@lit:rowStyle" disable-output-escaping="yes" />
      </xsl:if>
        ...
        <td id="{$gridName}l_{$parentRowLevel}{$rowIndex}" class="{$rowLabelClass}" >
          
          <xsl:attribute name="style">
            "text-align:center;vertical-align:middle;"
            <xsl:choose>
              <xsl:when test="@lit:rowSelectStyle">
                <xsl:value-of select="@lit:rowSelectStyle" disable-output-escaping="yes" />
              </xsl:when>
            </xsl:choose>
          </xsl:attribute>
            <xsl:choose>
			...
			<div id="{$gridName}_drs">
				...
				<table border="0" cellspacing="0" cellpadding="0" style="position:relative;table-layout:fixed;{$fixedScrollLeft}" height="100%">
					<colgroup>
						...
						<tr id="{$gridName}nfr_{$parentRowLevel}{$rowIndex}">
						<xsl:attribute name="style">
							<xsl:choose>
...
<xsl:template match="wbg:Group">
	...
	<tr id="{$gridName}gr_{$parentRowLevel}{$rowIndex}" groupRow="{@lit:groupRow}" level="{$parentRowLevel}{$rowIndex}">
